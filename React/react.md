...
> 리액트는 사용자 인터페이스를 구축하기 위한 자바스크립트 `라이브러리` 이다. 선언적인 뷰를 만 들 수 있어 코드의 가독성이 높고, 컴포넌트 기반으로 재사용 가능한 UI를 만들 수 있기 떄문에 개발 효율성이 높아진다. 주로 `Single Page Application`을 만들 때 사용되며 가상 돔을 활용한 성능향상 및 클라이언트 사이드 렌더링을 지원한다. `리액트는 컴포넌트에 기반한 접근 방식을 사용하므로, 재사용 가능성이 높은 컴포넌트를 개발할 수 있다.`
