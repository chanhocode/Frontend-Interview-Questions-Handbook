...
> 자바스크립트 ES6에서 도입된 특별한 종류의 함수로, 이 함수들은 호출될 때 함수 내의 모든 코드를 한 번에 실행하는 대신, `yield` 표현식을 `만날 때까지만 실행`하고 해당 상태에서 함수의 실행을 중지할 수 있습니다. 이후 `다시 함수를 호출`하면, 중단된 부분부터 `실행을 재개`합니다.
