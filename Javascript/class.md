...
> 자바스크립트의 클래스는 ES6에 도입되어 객체 지향 프로그래밍을 더욱 쉽고 명확하게 구현할 수 있게 한 구문이다. 클래스는 자바스크립의 기존 프로토타입 기반 상속을 보다 간결하고 이해하기 쉬운 문접으로 감싼 '문법적 설탕(Syntactic Sugar)'이라 하기도 한다.
...
> 프로토타입은 객체지향 프로그래밍의 핵심 개념 중 하나로, 객체 간에 속성과 메서드를 상속하기 위해 사용됩니다. 프로토타입은 모든 자바스크립트 객체가 가지고 있는 내부 속성으로, 다른 객체를 참조하는 숨겨진 링크입니다. 이 링크는 객체가 생성될 때 다른 객체로부터 상속받은 속성과 메서드를 찾는 데 사용됩니다.
