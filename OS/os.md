...
- 프로세스(Process)
  ...
  3.  `통신과 동기화`: 프로세스 간 통신(IPC)은 상대적으로 복잡하며, 파이프, 소켓, 공유 메모리 등의 방법을 사용한다.
...
- 선점형 스케줄링:
  - 라운드로빈: 각 프로세스는 동일한 할당 시간을 주고 그 시간 안에 끝나지 않으면 다시 준비 큐의 뒤로 가는 알고리즘
